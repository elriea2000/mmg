...
# ターゲットファイル
#fn="./snapshots/snapshot_202606201118.html"
if len(sys.argv)>1:
    fn=sys.argv[1]
else:
...
# 無視するレス番一覧（削除されたものとか）
ignore_id=[0,1,2,26,49,56,67,70,71,72,85,97,109,118,126]
...
for ic,card in enumerate(cards):
    ...
    card_mc = card.select_one("div.message-content")
    card_bc = card_mc.parent.next_sibling
    ...
    # 絵文字抽出
    btns = card_bc.findChildren("button",recursive=False)
    dbtns=[]
    for btn in btns:
        spans = btn.findChildren("span",recursive=False)
        bsrc = spans[0].findChild("img").attrs["src"]
